...
<template>
  ...
  <TheDialog
    ...
    :style="{ width: '25rem', color: '#FFFFFF' }"
  >
    <span class="text-surface-500 dark:text-surface-400 block mb-8 custom-dialog"
      >o numero do contador {{ numberObject.valorPrimo }} .</span
    >
    <div class="flex items-center gap-4 mb-4"></div>
    <div class="flex items-center gap-4 mb-8"></div>
  ...
  <div class="contadorClass">valor : {{ numberObject.contar }}</div>
  <div class="contadorClass">valor : {{ numberObject.verificarPar }}</div>
  <div class="contadorClass">valor : {{ numberObject.valorPrimo }}</div>
</template>
